...
-- ДОП_ЗАДАНИЕ: Вывести ФИО и квалификацию самого молодого препода КТ
SELECT EXTRACT(YEAR FROM AGE(Н_ЛЮДИ.ДАТА_СМЕРТИ, Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)) AS ВОЗРАСТ FROM Н_ЛЮДИ;
SELECT EXTRACT(YEAR FROM AGE(Н_ЛЮДИ.ДАТА_СМЕРТИ, Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)) AS ВОЗРАСТ FROM Н_ЛЮДИ;
SELECT Н_ЛЮДИ.ФАМИЛИЯ, Н_ЛЮДИ.ИМЯ, Н_ЛЮДИ.ОТЧЕСТВО, Н_КВАЛИФИКАЦИИ.НАИМЕНОВАНИЕ 
FROM Н_ЛЮДИ
...
WHERE Н_НАПР_СПЕЦ.НАИМЕНОВАНИЕ = 'Прикладная математика и информатика' 
